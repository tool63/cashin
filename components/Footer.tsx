  ...
  return (
    <footer className="bg-gray-100 dark:bg-gray-900 text-black dark:text-white mt-16">
      <div className="max-w-7xl mx-auto px-4 py-10 grid md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          {open["support"] && (
            <ul className="space-y-1 mt-2">
              <li>
                <Link href="/help">{t("footer.links.helpCenter")}</Link>
              </li>
              <li>
                <Link href="/faq">{t("footer.links.faq")}</Link>
              </li>
              <li>
                <Link href="/contact">{t("footer.links.contactUs")}</Link>
              </li>
            </ul>
          )}
        </div>
      ...
      <div className="border-t border-gray-300 dark:border-gray-700 text-center py-4 mt-8 text-sm">
        &copy; {new Date().getFullYear()} PayUp. {t("footer.rights")}
      </div>
    </footer>
  );
}
