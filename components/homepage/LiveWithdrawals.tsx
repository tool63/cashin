...
// Base users for worldwide flags
const baseUsers = [
...
export default function LiveWithdrawals() {
  ...
  useEffect(() => {
    ...
    const addNewWithdrawal = () => {
      ...
      setTimeout(addNewWithdrawal, nextInterval);
    };
    addNewWithdrawal();
  ...
  useEffect(() => {
    const interval = setInterval(() => setWithdrawals((prev) => [...prev]), 1000);
          ...
          <ul className="space-y-4 p-6">
            {withdrawals.map((w) => (
              ...
              >
                ...
                <span className="text-emerald-600 dark:text-emerald-400 font-bold text-center">{w.amount}</span>
                <span className="text-gray-500 dark:text-gray-400 text-center">{formatTime(w.joinedAt)}</span>
              </li>
            ))}
          </ul>
