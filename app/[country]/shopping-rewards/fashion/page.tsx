// app/[country]/(marketing)/shopping-rewards/fashion/page.tsx
...
const getCountrySpecificKeywords = (countryName: string, countryCode: string): string[] => {
  ...
  const baseKeywords = [
    `fashion cashback ${lowerCountry}`,
    `cashback on clothing ${lowerCountry}`,
    `shop fashion with cashback ${lowerCountry}`,
    `best fashion deals ${lowerCountry}`,
    `clothing shopping rewards ${lowerCountry}`,
    `cashback fashion stores ${lowerCountry}`,
    `save on clothes ${lowerCountry}`,
    `fashion discounts ${lowerCountry}`,
    `designer cashback ${lowerCountry}`,
    `sustainable fashion cashback ${lowerCountry}`,
  ];
  ...
  if (countryCode === "us") {
    ...
    baseKeywords.push(
      "fashion deals canada",
      "hudson bay cashback",
      "simons canada"
    );
  } else if (countryCode === "au") {
    baseKeywords.push(
      "fashion deals australia",
      "the iconic cashback",
      "myer fashion deals"
    );
  }
...
export async function generateMetadata({
  ...
  try {
    translation = await loadSectionTranslation(language, "shopping-rewards-fashion");
  } catch (error) {
    // Use defaults
  }
  ...
  const seoTitle = replaceCountry(
    rawTitle,
    `Fashion Cashback - Earn Up to 15% Back on Clothing in ${countryName} | Cashog`
  );

  const seoDescription = replaceCountry(
    rawDescription,
    `Shop fashion with cashback in ${countryName}. Earn up to 15% back at top stores like ASOS, Nordstrom, Macy's, Zara, and more. Get the best deals on clothing, shoes, accessories, and designer brands.`
  );
  ...
  return {
    ...
    keywords,
    alternates: {
      canonical: `https://cashog.com/${country}/shopping-rewards/fashion`,
    },
    openGraph: {
      title: seoTitle,
      description: seoDescription,
      url: `https://cashog.com/${country}/shopping-rewards/fashion`,
      siteName: "Cashog",
      type: "website",
...
export default async function FashionRewardsPage({
  params,
}: {
  params: Promise<{ country?: string }> | { country?: string };
}) {
  ...
  // Load translations
  const tData = await loadSectionTranslation(language, "shopping-rewards-fashion");
  ...
  const title = t(rawTitle, `Fashion Cashback - Earn Up to 15% Back`);
  const description = t(rawDescription, `Shop fashion with cashback in ${countryName}.`);

  const structuredData = generateJsonLd({
    path: `/${country}/shopping-rewards/fashion`,
    title,
    description,
    type: "low",
  });
  ...
  const heroData = {
    title: t(tData?.hero?.title, "Earn Cashback on Fashion"),
    subtitle: t(
      tData?.hero?.subtitle,
      `Get paid to shop for the latest trends. Earn up to 15% cashback at top fashion retailers in ${countryName}. From everyday essentials to luxury designer pieces.`
    ),
  };
  ...
  if (categoryData.categories.length === 0) {
    categoryData.categories = [
      ...
      },
      {
        name: "Men's Clothing",
        icon: "👔",
        description: "Suits, shirts, pants, jackets, and accessories",
        storeCount: 38,
      },
      {
        name: "Shoes",
        icon: "👠",
        description: "Sneakers, boots, heels, sandals, and loafers",
        storeCount: 42,
      },
      {
        name: "Accessories",
        icon: "👜",
        description: "Bags, watches, jewelry, belts, and hats",
        storeCount: 35,
      },
      {
        name: "Activewear",
        icon: "🏃‍♀️",
        description: "Gym clothes, yoga wear, and sports gear",
        storeCount: 28,
      },
      {
  ...
  const featuredStoresData = {
    title: t(tData?.featuredStoresTitle, "Top Fashion Stores"),
    ...
    featuredStoresData.stores = [
      {
        ...
        description: "Thousands of brands and styles for men and women",
        categories: ["Clothing", "Shoes", "Accessories"],
        features: ["Free returns", "Student discount", "Next-day delivery"],
        link: "/store/asos",
      },
      {
        name: "Nordstrom",
        ...
        description: "Premium fashion, shoes, and accessories",
        categories: ["Designer", "Shoes", "Beauty"],
        features: ["Free shipping", "Price match", "Curbside pickup"],
        link: "/store/nordstrom",
      },
      {
        name: "Macy's",
        logo: "⭐",
        cashbackRate: "Up to 6%",
        description: "Department store with endless fashion options",
        categories: ["Clothing", "Home", "Beauty"],
        features: ["Sales events", "Star rewards", "Free pickup"],
        link: "/store/macys",
      },
      {
        name: "Zara",
        ...
        features: ["New arrivals weekly", "Store pickup", "Easy returns"],
        link: "/store/zara",
      },
      {
        name: "Nike",
        ...
        categories: ["Activewear", "Shoes", "Sports"],
        features: ["Member exclusive", "Nike app", "Free shipping"],
        link: "/store/nike",
      },
      {
        name: "Revolve",
        ...
        description: "Emerging and established fashion brands",
        categories: ["Dresses", "Denim", "Active"],
        features: ["Free shipping", "VIP program", "Easy returns"],
        link: "/store/revolve",
      },
    ];
  }
  ...
  const topDealsData = {
    title: t(tData?.topDealsTitle, "Hot Fashion Deals"),
  ...
  if (topDealsData.deals.length === 0) {
    topDealsData.deals = [
      {
        title: "Winter Coat Sale",
        description: "Stay warm with up to 40% off winter jackets",
        originalPrice: "$199",
        salePrice: "$119",
        cashbackRate: "8%",
        store: "Nordstrom",
        image: "🧥",
        link: "/deal/winter-coat-sale",
      },
      {
        title: "Sneaker Clearance",
        description: "Last season's styles at unbeatable prices",
        originalPrice: "$120",
        salePrice: "$69",
        cashbackRate: "10%",
        store: "Nike",
        image: "👟",
        link: "/deal/sneaker-clearance",
      },
      {
        title: "Designer Handbags",
        description: "Luxury bags starting at $199",
        originalPrice: "$450",
        salePrice: "$199",
        cashbackRate: "12%",
        store: "Revolve",
        image: "👜",
        link: "/deal/designer-handbags",
      },
    ];
  }

  const tipsData = {
    title: t(tData?.tipsTitle, "Smart Fashion Shopping Tips"),
    ...
    tipsData.tips = [
      {
        ...
        description: "Combine cashback with store sales, coupons, and student discounts for maximum savings.",
        icon: "💰",
      },
      {
        title: "Check Return Policies",
        description: "Review return policies before buying, especially for final sale or clearance items.",
        icon: "🔄",
      },
      {
        title: "Sign Up for Newsletters",
        description: "Get exclusive coupons and early access to sales by joining store email lists.",
        icon: "📧",
      },
      {
        title: "Size Guides Matter",
        description: "Use store size guides and check reviews to ensure the perfect fit the first time.",
        icon: "📏",
      },
    ];
  }
  ...
  const compareData = {
    ...
    description: t(tData?.compare?.description, "See how much you can earn at top fashion stores"),
    ...
    seasonalPromosData.promos = [
      {
        ...
        description: "Up to 60% off on clearance items",
        endDate: "Jan 31, 2025",
        link: "/promos/end-of-season",
      },
      {
        title: "Holiday Fashion Event",
        description: "Extra 20% off + cashback on all orders",
        endDate: "Dec 25, 2025",
        code: "HOLIDAY20",
        link: "/promos/holiday-fashion",
      },
    ];
  }
    ...
    brandSpotlightData.brands = [
      {
        name: "Levi's",
        logo: "👖",
        description: "Iconic denim and casual wear for all",
        cashbackRate: "Up to 8%",
        featuredProducts: ["501 Original Jeans", "Trucker Jacket", "Graphic Tees"],
        link: "/brand/levis",
      },
      {
        name: "Adidas",
        logo: "👕",
        description: "Sportswear and lifestyle fashion",
        cashbackRate: "Up to 10%",
        featuredProducts: ["Ultraboost", "Stan Smith", "Track Suits"],
        link: "/brand/adidas",
      },
    ];
  }
  ...
  const buyingGuidesData = {
    title: t(tData?.buyingGuidesTitle, "Fashion Buying Guides"),
  ...
  if (buyingGuidesData.guides.length === 0) {
    buyingGuidesData.guides = [
      {
        title: "The Perfect Jean Fit Guide",
        description: "Find your ideal jeans based on body type and style preference",
        readTime: "5 min",
        link: "/guide/jean-fit",
      },
      {
        title: "Sustainable Fashion Shopping",
        description: "How to shop ethically while earning cashback",
        readTime: "7 min",
        link: "/guide/sustainable-fashion",
      },
      {
        title: "Dressing for Every Season",
        description: "Build a versatile wardrobe that works year-round",
        readTime: "6 min",
        link: "/guide/seasonal-dressing",
      },
    ];
  }
  ...
  const priceMatchData = {
    ...
    description: t(tData?.priceMatch?.description, "We'll help you find the best cashback rates on fashion"),
    buttonText: t(tData?.priceMatch?.buttonText, "Compare Prices"),
  };

  const sizeGuideData = {
    title: t(tData?.sizeGuide?.title, "Need Help with Sizing?"),
    description: t(tData?.sizeGuide?.description, "Use our comprehensive size guides to find the perfect fit every time"),
    buttonText: t(tData?.sizeGuide?.buttonText, "View Size Guides"),
  };

  const faqData = {
    title: t(tData?.faq?.title, `Fashion Cashback FAQ - ${countryName}`),
  ...
  if (faqData.items.length === 0) {
    faqData.items = [
      {
        q: "Which fashion stores offer the highest cashback?",
        a: "Cashback rates vary by store and season. Nike often offers 12%, ASOS up to 10%, and Nordstrom up to 8%. Check our store listings for current rates.",
      },
      {
        q: "Can I earn cashback on sale or clearance items?",
        a: "Yes! Most stores offer cashback on sale and clearance items. However, check individual store terms as some exclusions may apply.",
      },
      {
        q: "How long does fashion cashback take to confirm?",
        a: "Fashion cashback typically takes 30-60 days to confirm due to return periods. This protects against returns and exchanges.",
      },
      {
        q: "What happens if I return an item?",
        a: "If you return an item, the cashback will be deducted from your account. Wait until after the return period to withdraw cashback.",
      },
      {
        q: "Can I use coupons with cashback?",
        a: "Yes! In most cases, you can stack coupons and promo codes with cashback. Just make sure to click through our link first.",
      },
      {
        q: "Do size exchanges affect my cashback?",
        a: "Exchanges for different sizes usually don't affect cashback as long as the order value remains the same. However, returns for refunds will reduce your cashback.",
      },
    ];
  }

  const finalData = {
    title: t(tData?.final?.title, "Start Saving on Fashion Today"),
    subtitle: t(tData?.final?.subtitle, "Join thousands of smart shoppers earning cashback on their clothing purchases"),
    buttonText: t(tData?.final?.buttonText, "Shop Fashion Now"),
  };
        ...
        <OpeningStyle delay={0.1}>
          ...
          >
            <div className="text-center mb-16">
              ...
              <div
                className="w-24 h-1 bg-gradient-to-r from-pink-400 to-purple-500 mx-auto mt-4 rounded-full"
                aria-hidden="true"
              />
            </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {topDealsData.deals.map((deal, index) => (
                  ...
                  >
                    <div className="aspect-w-16 aspect-h-9 bg-gradient-to-br from-pink-400 to-purple-500 flex items-center justify-center p-8">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {tipsData.tips.map((tip, index) => (
                <div
                  key={index}
                  className="bg-gradient-to-br from-pink-50 to-purple-50 dark:from-gray-800 dark:to-gray-900 rounded-xl p-6 text-center"
                >
                  <div className="text-5xl mb-4" aria-hidden="true">
                    {tip.icon}
                  </div>
      ...
      <CircleBorder>
        <OpeningStyle delay={0.1}>
          ...
          >
            ...
            <div
              className="w-24 h-1 bg-gradient-to-r from-pink-400 to-purple-500 mx-auto mt-4 rounded-full mb-8"
              aria-hidden="true"
            />
      ...
      <CircleBorder>
        <OpeningStyle delay={0.1}>
          ...
          >
            ...
            <PrimaryCTA
              href="/size-guides"
              translationKey={sizeGuideData.buttonText}
              observer={true}
            />
          </section>
        </OpeningStyle>
