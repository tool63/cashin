...
export async function generateMetadata({
  ...
  const seoTitle = replaceCountry(
    rawTitle,
    `Discount Vouchers & Promo Codes in ${countryName} - Save Money Shopping | Cashog`
  );

  const seoDescription = replaceCountry(
    rawDescription,
    `Get the best discount vouchers and promo codes in ${countryName}. Save up to 70% at 5000+ stores. Free access to exclusive deals and coupons. Start saving today!`
  );
...
export default async function VouchersPage({
  ...
  const title = t(rawTitle, `Discount Vouchers in ${countryName}`);
  const description = t(rawDescription, `Save money with discount vouchers in ${countryName}.`);
