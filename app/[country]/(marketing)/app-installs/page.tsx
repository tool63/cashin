...
const getCountrySpecificKeywords = (countryName: string, countryCode: string): string[] => {
  ...
  const baseKeywords = [
    `install apps get paid ${lowerCountry}`,
    `get paid to install apps ${lowerCountry}`,
    `paid app installs ${lowerCountry}`,
    `earn money installing apps ${lowerCountry}`,
    `app install rewards ${lowerCountry}`,
    `get paid for downloading apps ${lowerCountry}`,
    `install and earn ${lowerCountry}`,
    `paid to download apps ${lowerCountry}`,
    `app install cash rewards ${lowerCountry}`,
    `download apps earn money ${lowerCountry}`,
    `best app install sites ${lowerCountry}`,
    `legit app install apps ${lowerCountry}`,
    `make money downloading apps ${lowerCountry}`,
    `app install tasks ${lowerCountry}`,
    `paid app download offers ${lowerCountry}`,
    `install apps for cash instantly ${lowerCountry}`,
    `highest paying app installs ${lowerCountry}`,
    `earn $1 per app install ${lowerCountry}`,
    `app install referral programs ${lowerCountry}`,
    `get paid to test apps ${lowerCountry}`,
    `mobile app install rewards ${lowerCountry}`,
    `iOS app install paid ${lowerCountry}`,
    `Android app install earn money ${lowerCountry}`,
  ];

  // Add country-specific variations
  if (countryCode === "us") {
    baseKeywords.push(
      "get paid to install apps usa",
      "app install jobs for americans",
      "highest paying app installs usa"
    );
  } else if (countryCode === "gb") {
    baseKeywords.push(
      "get paid to install apps uk",
      "app install money uk",
      "best app install sites for british users"
    );
  } else if (countryCode === "ca") {
    baseKeywords.push(
      "get paid to install apps canada",
      "canadian app install rewards",
      "earn cad from app installs"
    );
  } else if (countryCode === "au") {
    baseKeywords.push(
      "get paid to install apps australia",
      "app install money australia",
      "earn aud from downloading apps"
    );
  }
...
export async function generateMetadata({
  ...
  try {
    translation = await loadSectionTranslation(language, "app-installs");
  } catch (error) {
    // Use defaults
  }
  ...
  const seoTitle = replaceCountry(
    rawTitle,
    `Get Paid to Install Apps in ${countryName} - Earn $0.50-$5 Per App | Cashog`
  );

  const seoDescription = replaceCountry(
    rawDescription,
    `Join 150,000+ members earning cash in ${countryName} by installing free apps. Download apps, try them for 30 seconds, and get paid via PayPal or gift cards. Free to join!`
  );
  ...
  return {
    ...
    keywords,
    alternates: {
      canonical: `https://cashog.com/${country}/app-installs`,
    },
    openGraph: {
      title: seoTitle,
      description: seoDescription,
      url: `https://cashog.com/${country}/app-installs`,
      siteName: "Cashog",
      type: "website",
...
export default async function AppInstallsPage({
  ...
  // Load translations
  const tData = await loadSectionTranslation(language, "app-installs");
  ...
  const title = t(rawTitle, `Get Paid to Install Apps in ${countryName} - Earn Cash`);
  const description = t(rawDescription, `Join 150,000+ members earning cash in ${countryName} by installing free apps.`);
  ...
  const heroData = {
    title: t(tData?.hero?.title, `Get Paid to Install Apps in ${countryName}`),
    subtitle: t(
      tData?.hero?.subtitle,
      `Join 150,000+ members earning real cash in ${countryName} by installing free apps. Download, try, and earn - it's that simple!`
    ),
  };

  const statsData = {
    title: t(tData?.statsTitle, "Trusted by Thousands"),
    appsInstalled: tData?.stats?.appsInstalled || "10M+",
    appsInstalledLabel: tData?.stats?.appsInstalledLabel || "Apps Installed",
    avgPayout: tData?.stats?.avgPayout || "$1.50",
    avgPayoutLabel: tData?.stats?.avgPayoutLabel || "Average Per App",
    activeUsers: tData?.stats?.activeUsers || "150K+",
  ...
  const appCategoriesData = (tData?.appCategories || []).map((category) => ({
    ...category,
    title: t(category.title, category.title),
    description: t(category.description, category.description),
  }));

  const featuredAppsData = (tData?.featuredApps || []).map((app) => ({
    ...app,
    title: t(app.title, app.title),
  }));
  ...
  const faqData = {
    title: t(tData?.faq?.title, `Get Paid to Install Apps in ${countryName} - FAQ`),
  ...
  const finalData = {
    ...
    subtitle: t(
      ...
      `Join 150,000+ members already getting paid in ${countryName}. Sign up for free and install your first paid app today!`
    ),
  };
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              <div className="bg-gradient-to-br from-yellow-50 to-green-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl">
                <div className="text-3xl md:text-4xl font-bold text-green-600">
                  {statsData.appsInstalled}
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                  {statsData.appsInstalledLabel}
                </div>
              </div>
              <div className="bg-gradient-to-br from-yellow-50 to-green-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl">
                <div className="text-3xl md:text-4xl font-bold text-green-600">
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.categoriesTitle, "Popular App Categories")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
                ...
                <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-6">
                  {t(tData?.categoriesSubtitle, "Earn by installing these types of apps")}
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {appCategoriesData.map((category, index) => (
                  <div
                    key={index}
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.featuredAppsTitle, "High-Paying App Installs Available Now")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
                />
                <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-6">
                  {t(tData?.featuredAppsSubtitle, "Limited spots - install these apps today")}
                </p>
              </div>
              <div className="max-w-4xl mx-auto space-y-4">
                {featuredAppsData.map((app, index) => (
                  <div
                    key={index}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                              difficultyColors[app.difficulty]
                            }`}
                          >
                            {app.difficulty}
                          </span>
                        ...
                        <div className="flex flex-wrap gap-4 text-xs">
                          ...
                          <span className="text-orange-600 dark:text-orange-400">
                            🎯 {app.spotsLeft.toLocaleString()} spots left
                          </span>
                        </div>
                      </div>
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.benefitsTitle, "Why Install Apps for Cash")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.tipsTitle, "Tips to Maximize Your Earnings")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
