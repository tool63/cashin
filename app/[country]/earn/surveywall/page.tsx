// app/[country]/(marketing)/surveywall/page.tsx
...
interface TranslationSection {
  ...
  featuredSurveysTitle?: string;
  featuredSurveysSubtitle?: string;
...
const getCountrySpecificKeywords = (countryName: string, countryCode: string): string[] => {
  ...
  const baseKeywords = [
    `surveywall ${lowerCountry}`,
    `paid surveys ${lowerCountry}`,
    `get paid for surveys ${lowerCountry}`,
    `surveywall earn money ${lowerCountry}`,
    `best survey sites ${lowerCountry}`,
    `online surveys get paid ${lowerCountry}`,
    `paid survey panel ${lowerCountry}`,
    `survey rewards ${lowerCountry}`,
    `highest paying surveys ${lowerCountry}`,
    `legit survey sites ${lowerCountry}`,
    `surveywall paypal ${lowerCountry}`,
    `daily surveys earn cash ${lowerCountry}`,
    `market research surveys ${lowerCountry}`,
    `consumer survey panels ${lowerCountry}`,
    `paid opinion surveys ${lowerCountry}`,
    `survey aggregator ${lowerCountry}`,
    `best surveywall platform ${lowerCountry}`,
    `survey cash rewards ${lowerCountry}`,
    `take surveys for money ${lowerCountry}`,
    `surveywall instant payout ${lowerCountry}`,
  ];
  ...
  if (countryCode === "us") {
    baseKeywords.push(
      "surveywall usa",
      "best paid surveys for americans",
      "highest paying surveys usa"
    );
  } else if (countryCode === "gb") {
    baseKeywords.push(
      "surveywall uk",
      "paid surveys uk",
      "earn pounds taking surveys"
    );
  } else if (countryCode === "ca") {
    baseKeywords.push(
      "surveywall canada",
      "canadian paid surveys",
      "earn cad taking surveys"
    );
  } else if (countryCode === "au") {
    baseKeywords.push(
      "surveywall australia",
      "australian paid surveys",
      "earn aud taking surveys"
    );
  } else if (countryCode === "de") {
    baseKeywords.push(
      "surveywall deutschland",
    ...
    baseKeywords.push(
      ...
      "gagner de l'argent avec des sondages"
    );
...
export async function generateMetadata({
  ...
  try {
    translation = await loadSectionTranslation(language, "surveywall");
  } catch (error) {
    // Use defaults
  }
  ...
  const seoTitle = replaceCountry(
    rawTitle,
    `SurveyWall in ${countryName} - Get Paid $1-$50 Per Survey | Cashog`
  );

  const seoDescription = replaceCountry(
    rawDescription,
    `Access 30+ survey providers through our surveywall in ${countryName}. Share your opinion & earn $500+ monthly. Highest paying surveys guaranteed!`
  );
  ...
  return {
    ...
    keywords,
    alternates: {
      canonical: `https://cashog.com/${country}/surveywall`,
    },
    openGraph: {
      title: seoTitle,
      description: seoDescription,
      url: `https://cashog.com/${country}/surveywall`,
      siteName: "Cashog",
      type: "website",
...
export default async function SurveywallPage({
  params,
}: {
  params: Promise<{ country?: string }> | { country?: string };
}) {
  ...
  // Load translations
  const tData = await loadSectionTranslation(language, "surveywall");
  ...
  const title = t(rawTitle, `SurveyWall in ${countryName} - Get Paid for Your Opinion`);
  const description = t(rawDescription, `Access multiple survey providers through our surveywall in ${countryName}.`);

  const structuredData = generateJsonLd({
    path: `/${country}/surveywall`,
    title,
    description,
    type: "low",
  });
  ...
  const heroData = {
    title: t(tData?.hero?.title, `Get Paid for Your Opinion in ${countryName}`),
    subtitle: t(
      tData?.hero?.subtitle,
      `Access 30+ premium survey providers in ${countryName}. Share your opinion on products, services, and brands. Earn $500+ monthly from a single dashboard!`
    ),
  };

  const statsData = {
    title: t(tData?.statsTitle, "SurveyWall Statistics"),
    surveysCompleted: tData?.stats?.surveysCompleted || "20M+",
    surveysCompletedLabel: tData?.stats?.surveysCompletedLabel || "Surveys Completed",
    avgPayout: tData?.stats?.avgPayout || "$3.50",
    avgPayoutLabel: tData?.stats?.avgPayoutLabel || "Average Per Survey",
    activeUsers: tData?.stats?.activeUsers || "200K+",
    activeUsersLabel: tData?.stats?.activeUsersLabel || "Active Members",
    totalPaid: tData?.stats?.totalPaid || "$40M+",
    totalPaidLabel: tData?.stats?.totalPaidLabel || "Total Paid",
  };

  const surveyCategoriesData = (tData?.surveyCategories || []).map((category) => ({
    ...category,
    title: t(category.title, category.title),
    description: t(category.description, category.description),
  }));

  const featuredSurveysData = (tData?.featuredSurveys || []).map((survey) => ({
    ...survey,
  ...
  const faqData = {
    title: t(tData?.faq?.title, `SurveyWall in ${countryName} - FAQ`),
  ...
  const finalData = {
    title: t(tData?.final?.title, `Ready to Share Your Opinion in ${countryName}?`),
    subtitle: t(
      tData?.final?.subtitle,
      `Join 200,000+ members already earning through our surveywall in ${countryName}. Sign up for free and start taking paid surveys today!`
    ),
  };
      ...
      <CircleBorder>
        <OpeningStyle delay={0.1}>
          ...
          >
            ...
            <PrimaryCTA
              href="/signup"
              translationKey="start_surveying_now"
              observer={true}
            />
          </section>
        </OpeningStyle>
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.categoriesTitle, "Survey Categories")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
                ...
                <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-6">
                  {t(tData?.categoriesSubtitle, "Earn by sharing your opinion on these topics")}
                </p>
              </div>
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.featuredSurveysTitle, "High-Paying Surveys Available Now")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
                />
                <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-6">
                  {t(tData?.featuredSurveysSubtitle, "Limited spots - take these surveys today")}
                </p>
              </div>
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-4 text-xs">
                          ...
                          <span className="text-orange-600 dark:text-orange-400">
                            🎯 {survey.spotsLeft.toLocaleString()} spots left
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        <PrimaryCTA
                          href="/signup"
                          translationKey="take_survey_now"
                          observer={false}
                        />
                      </div>
                    </div>
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.benefitsTitle, "Why Choose Our SurveyWall")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.tipsTitle, "Tips to Maximize Survey Earnings")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.testimonialsTitle, "What Our Survey Takers Say")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
