// app/[country]/(marketing)/complete-offers/page.tsx
...
const getCountrySpecificKeywords = (countryName: string, countryCode: string): string[] => {
  ...
  const baseKeywords = [
    `complete offers get paid ${lowerCountry}`,
    `paid offers ${lowerCountry}`,
    `earn money completing offers ${lowerCountry}`,
    `get paid for completing offers ${lowerCountry}`,
    `offer completion jobs ${lowerCountry}`,
    `paid survey offers ${lowerCountry}`,
    `sign up offers get paid ${lowerCountry}`,
    `trial offers earn money ${lowerCountry}`,
    `complete tasks get paid ${lowerCountry}`,
    `highest paying offers ${lowerCountry}`,
    `legit offer platforms ${lowerCountry}`,
    `complete offers paypal ${lowerCountry}`,
    `earn $50 completing offers ${lowerCountry}`,
    `offer wall rewards ${lowerCountry}`,
    `daily offer earnings ${lowerCountry}`,
    `best offer completion sites ${lowerCountry}`,
    `get paid for downloads ${lowerCountry}`,
    `sign up bonus offers ${lowerCountry}`,
    `free trial offers get paid ${lowerCountry}`,
    `offer completion from home ${lowerCountry}`,
  ];

  // Add country-specific variations
  if (countryCode === "us") {
    baseKeywords.push(
      "complete offers get paid usa",
      "paid offers for americans",
      "highest paying offers usa"
    );
  } else if (countryCode === "gb") {
    baseKeywords.push(
      "complete offers get paid uk",
      "paid offers uk",
      "earn pounds completing offers"
    );
  } else if (countryCode === "ca") {
    baseKeywords.push(
      "complete offers get paid canada",
      "canadian paid offers",
      "earn cad completing offers"
    );
  } else if (countryCode === "au") {
    baseKeywords.push(
      "complete offers get paid australia",
      "australian paid offers",
      "earn aud completing offers"
    );
  } else if (countryCode === "de") {
    baseKeywords.push(
      "angebote abschließen geld verdienen deutschland",
      "bezahlte angebote deutschland",
      "aufgaben erledigen geld verdienen"
    );
  } else if (countryCode === "fr") {
    baseKeywords.push(
      "compléter des offres rémunérées france",
      "offres payées france",
      "gagner de l'argent avec des offres"
    );
  } else if (countryCode === "es") {
    baseKeywords.push(
      "completar ofertas pagadas españa",
      "ofertas pagadas españa",
      "ganar dinero completando ofertas"
    );
  }
...
export async function generateMetadata({
  ...
  try {
    translation = await loadSectionTranslation(language, "complete-offers");
  } catch (error) {
    // Use defaults
  }
  ...
  const seoTitle = replaceCountry(
    rawTitle,
    `Complete Offers & Get Paid in ${countryName} - Earn $5-$100 Per Offer | Cashog`
  );

  const seoDescription = replaceCountry(
    rawDescription,
    `Join 100,000+ members earning real cash in ${countryName} by completing offers, surveys, and trials. Get paid via PayPal or gift cards. Free to join!`
  );
  ...
  return {
    ...
    keywords,
    alternates: {
      canonical: `https://cashog.com/${country}/complete-offers`,
    },
    openGraph: {
      title: seoTitle,
      description: seoDescription,
      url: `https://cashog.com/${country}/complete-offers`,
      siteName: "Cashog",
      type: "website",
...
export default async function CompleteOffersPage({
  params,
}: {
  params: Promise<{ country?: string }> | { country?: string };
}) {
  ...
  // Load translations
  const tData = await loadSectionTranslation(language, "complete-offers");
  ...
  const title = t(rawTitle, `Complete Offers & Get Paid in ${countryName} - Earn Cash`);
  const description = t(rawDescription, `Join members earning cash in ${countryName} by completing offers.`);

  const structuredData = generateJsonLd({
    path: `/${country}/complete-offers`,
    title,
    description,
    type: "low",
  });
  ...
  const heroData = {
    title: t(tData?.hero?.title, `Get Paid to Complete Offers in ${countryName}`),
    subtitle: t(
      tData?.hero?.subtitle,
      `Join 100,000+ members earning real cash in ${countryName} by completing offers, surveys, and trials. Simple tasks that pay instantly!`
    ),
  };

  const statsData = {
    title: t(tData?.statsTitle, "Trusted by Thousands"),
    offersCompleted: tData?.stats?.offersCompleted || "10M+",
    offersCompletedLabel: tData?.stats?.offersCompletedLabel || "Offers Completed",
    avgPayout: tData?.stats?.avgPayout || "$15.00",
    avgPayoutLabel: tData?.stats?.avgPayoutLabel || "Average Per Offer",
    activeUsers: tData?.stats?.activeUsers || "100K+",
    activeUsersLabel: tData?.stats?.activeUsersLabel || "Active Members",
    totalPaid: tData?.stats?.totalPaid || "$30M+",
    totalPaidLabel: tData?.stats?.totalPaidLabel || "Total Paid to Members",
  };

  const offerCategoriesData = (tData?.offerCategories || []).map((category) => ({
    ...category,
  ...
  const faqData = {
    title: t(tData?.faq?.title, `Complete Offers & Get Paid in ${countryName} - FAQ`),
  ...
  const finalData = {
    ...
    subtitle: t(
      ...
      `Join 100,000+ members already getting paid in ${countryName}. Sign up for free and start completing paid offers today!`
    ),
  };
      ...
      <CircleBorder>
        <OpeningStyle delay={0.1}>
          ...
          >
            ...
            <PrimaryCTA
              href="/signup"
              translationKey="start_completing_offers"
              observer={true}
            />
          </section>
        </OpeningStyle>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
              <div className="bg-gradient-to-br from-yellow-50 to-green-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl">
                <div className="text-3xl md:text-4xl font-bold text-green-600">
                  {statsData.offersCompleted}
                </div>
                <div className="text-sm text-gray-600 dark:text-gray-400 mt-2">
                  {statsData.offersCompletedLabel}
                </div>
              </div>
              <div className="bg-gradient-to-br from-yellow-50 to-green-50 dark:from-gray-800 dark:to-gray-900 p-6 rounded-xl">
                <div className="text-3xl md:text-4xl font-bold text-green-600">
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.categoriesTitle, "Popular Offer Categories")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
                ...
                <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-6">
                  {t(tData?.categoriesSubtitle, "Earn by completing these types of offers")}
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {offerCategoriesData.map((category, index) => (
                  <div
                    key={index}
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.featuredOffersTitle, "High-Paying Offers Available Now")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
                />
                <p className="text-lg text-gray-600 dark:text-gray-400 max-w-2xl mx-auto mt-6">
                  {t(tData?.featuredOffersSubtitle, "Limited spots - complete these offers today")}
                </p>
              </div>
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      ...
                      <div className="text-right">
                        ...
                        <PrimaryCTA
                          href="/signup"
                          translationKey="complete_offer_now"
                          observer={false}
                        />
                      </div>
                    </div>
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.benefitsTitle, "Why Complete Offers on Cashog")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
            ...
            >
              <div className="text-center mb-16">
                <h2
                  ...
                  className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-white mb-4"
                >
                  {t(tData?.tipsTitle, "Tips to Maximize Your Offer Earnings")}
                </h2>
                <div
                  className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-green-500 mx-auto mt-4 rounded-full"
                  aria-hidden="true"
